...
- [x] **Wave 2 (v2) STARTED — `mini-window` Phase 2 (Spec) COMPLETE** — spec **approved**; ADR-0003 (single-window two-mode); 18 ACs + 9 NFRs; **28 test cases** (`tests/cases/mini-window.md`) + manual checklist. All open questions resolved.
- 👉 **Next action:** `/implement mini-window` — **but run the ADR-0003 macOS spike-gate FIRST** (drag-after-frameless · stays-above-focused-app · close-intercept-hides-with-tray-updating; + shared `JourneyGame` survives re-parenting). See §7.
...
**`local-stats` Phase 3 (Build) is COMPLETE** — the spec is approved, the full slice is implemented, and
the suite is green (**473 tests**). On disk: `lib/features/stats/{domain,data,presentation}/`, the
`ActivityTicker.onSnapshot` seam + `main.dart` wiring (shipped engine untouched), the two OS adapters behind
interfaces, +137 unit/cubit tests, widget/integration tests, and `tests/cases/local-stats-manual-checklist.md`.
`/self-review` already found and fixed 2 blockers (B1 backwards-clock double-count, B2 emit-after-close) plus
an AC-19 closed-across-midnight defect, each with a regression test.

**🎉 `local-stats` is SHIPPED (2026-06-24) — Wave 1 (v1) is COMPLETE.** All five v1 slices
(`activity-detection` · `journey-engine` · `journey-view` · `route-progress` · `local-stats`) are shipped and
in `planning/done/`. Green report `tests/_runner/reports/local-stats/20260624-132008/` (191/191 in-scope,
484/484 whole-package), all ACs ticked, `/privacy-audit` PASS, spec `shipped`.

**🚀 Wave 2 (v2) STARTED — `mini-window` Phase 2 (Spec) is in progress.** `/new-feature mini-window` ran:
`specs/mini-window/` scaffolded from the template; the spec's Problem / User & outcome / Scope (In/Out) /
Constraints were drafted from Kevin's 4 scoping decisions (PiP = **compact Flame scene** reusing `journey-view` ·
tray = **status + quick actions** · close button = **hide-to-tray, keep tracking** · v1 window behaviours =
**remember position · always-on-top toggle · draggable/frameless**; **launch-at-startup is OUT** — `local-stats`
owns it). `product-domain-expert` proposed **18 ACs (AC-1..18) + 9 NFRs** in `acceptance-criteria.md` (single
source of truth, zero new privacy surface, macOS+Windows parity via `window_manager`+`tray_manager` with a
mock path). The spec carries **12 open questions** (6 original + 6 surfaced by domain framing, each with a
recommended default). `planning/active/mini-window.md` created (Phase ledger at Phase 2).

**All `mini-window` open questions are now RESOLVED** (Kevin's review 2026-06-24): YouTube-style
**user-invoked, mutually-exclusive** PiP (enter PiP → main hides to dock; open main → PiP dismissed) ·
always-present menu-bar icon (hide-to-tray, keep tracking) · **fixed compact size** (persist position
only) · static tray icon · one-time hide-to-tray hint · launch-at-startup OUT (`local-stats` owns it).
Wiring settled by `system-architect` → **Option A: single window, two modes** (one engine/Bloc/scene;
window transforms full ⇄ compact; lift `JourneyGame` to a shared owner). Recorded in **ADR-0003**
(`docs/architecture/decisions/0003-mini-window-single-window-two-mode.md`); `overview.md` v2 window model
updated. Spec `Status: in-review` (ready to approve).

**`mini-window` Phase 2 (Spec) is COMPLETE** — spec **approved**; ADR-0003 recorded; `product-domain-expert`
gave 18 ACs + 9 NFRs; `test-case-designer` gave **28 test cases** (`tests/cases/mini-window.md`) + a manual
checklist (`tests/cases/mini-window-manual-checklist.md`) with full AC/NFR→TC traceability.

**👉 Immediate next action — `/implement mini-window`, spike-gate FIRST.**
Per ADR-0003, before committing the build prove on **one real macOS window**: (a) `startDragging` works
after `setAsFrameless`/hidden title bar, (b) the window stays **above a different focused app** (macOS
NSWindow level / Spaces), (c) close-intercept (`setPreventClose` + `hide()`) keeps the process alive with the
tray still updating; secondary — the lifted shared `JourneyGame` survives re-parenting (IndexedStack/Offstage).
`/implement` will also use `/source-assets` (static tray icon art) + `/self-review`, and `/review-code` will run
`/privacy-audit` (zero new user-data surface is the headline NFR). Deferred by design: NFR-2 on-device fps,
NFR-9 Windows runtime (authored to parity now, verified before any Windows release).

**Remaining Wave-2 slices not yet started** (wave discipline — promote when ready): `journey-energy-model`
(`/capture-idea` → `/new-feature`), `map-geographic` (already unblocked — reuses route-progress's chain model
behind `flutter_map` tiles), `team-leaderboard` (needs a backend — its own sub-epic).
