...
- [ ] AC-1 (P0) — PiP mirrors the same Bloc, embeds the journey-view scene: **Given** the app is
...
- [ ] AC-2 (P0) — PiP stops when the Bloc says stopped: **Given** the Bloc emits `state = idle` or
...
- [ ] AC-3 (P0) — PiP starts/stops within one tick (mirrors journey-view binding): **Given** an
...
- [ ] AC-4 (P0) — distance + active/idle readout reflects the Bloc, not its own math: **Given** the
...
- [ ] AC-5 (P0) — first-frame / pre-state default is parked: **Given** the PiP is shown before the
...
- [ ] AC-6 (P0) — user-invoked PiP collapse, mutually exclusive with main (YouTube-style): **Given** the
...
- [ ] AC-7 (P2) — optional always-on-top off/on toggle: **Given** an (optional) always-on-top toggle,
...
- [ ] AC-8 (P0) — remembered position across restart: **Given** the user drags the (fixed-size) PiP to a
...
- [ ] AC-9 (P0) — exactly one engine/ticker/scene, shared by both windows (code inspection): **Given**
...
- [ ] AC-10 (P0) — PiP owns no journey logic and reads no OS signals about the user (code inspection):
...
- [ ] AC-11 (P0) — tray icon present and reflects state: **Given** the app is running, **When** the
...
- [ ] AC-12 (P0) — tray menu quick actions exist and work: **Given** the tray menu is open, **When**
...
- [ ] AC-13 (P1) — tray status line reflects journey state (if cheap): **Given** the tray menu is open,
...
- [ ] AC-14 (P2) — menu items reflect the current mode: **Given** the app is in full (main) mode or in
...
- [ ] AC-15 (P0) — main-window close hides to tray and keeps tracking: **Given** the app is tracking
...
- [ ] AC-16 (P0) — restore from tray, full exit only via Quit: **Given** the main window has been
...
- [ ] AC-17 (P1) — first-run hide-to-tray discoverability: **Given** the user clicks the main window's
...
- [ ] AC-18 (P0) — PiP is not auto-shown on close-to-tray: **Given** the user clicks the main window's
...
- [ ] NFR-1 (P0) — animation paused (not just hidden) when stopped or not visible: **Given** the Bloc
...
- [ ] NFR-2 (P1) — smooth compact scene, no added jank: **Given** the PiP is visible while `active`,
...
- [ ] NFR-3 (P1) — reduced motion honoured in the PiP: **Given** the OS/app "reduce motion" preference
...
- [ ] NFR-4 (P0, headline) — zero new user-data surface; passes `/privacy-audit`: **Given** a
...
- [ ] NFR-5 (P0) — no disqualifying new dependency: **Given** the dependencies this slice introduces
...
- [ ] NFR-6 (P1) — tray menu actions are keyboard / screen-reader reachable: **Given** the tray menu,
...
- [ ] NFR-7 (P0) — authored to macOS + Windows parity via window_manager + tray_manager: **Given** the
...
- [ ] NFR-8 (P0) — mock/dev path enables headless/widget tests with no real OS window: **Given** the
...
- [ ] NFR-9 (P1) — on-device Windows verification may be deferred, but parity is authored now:
